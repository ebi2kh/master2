...
class Actor(nn.Module):
    ...
    Actor Network
    """
    def __init__(self, state_dim, emb_dim, action_dim, init_std=1e0):
        super(Actor, self).__init__()
        # ****** My Code ****** #        
        # emb_dim = [256, 512, 512, 256]
        self.model_size = [state_dim] + emb_dim + [action_dim]
        # ****** My Code ****** #
        # Input normalization using LayerNorm (works well even with batch size 1)
        ...
        for i in range(len(self.model_size) - 2):
            ...
            # added code
            # bn = nn.LayerNorm(self.model_size[i + 1])
            # self.model.add_module(f'bn{i}', bn)
            # end of added code
...
class Critic(nn.Module):
    ...
    def __init__(self, state_dim, emb_dim):
        ...
        for i in range(len(self.model_size) - 2):
            self.model.add_module(f'fc{i}', init_weights(nn.Linear(self.model_size[i], self.model_size[i + 1], bias=True)))
            # added code
            print(self.model_size[i], self.model_size[i+1])
            # bn = nn.LayerNorm(self.model_size[i + 1])
            # self.model.add_module(f'bn{i}', bn)
            # end of added code
            self.model.add_module(f'act{i}', nn.ReLU())
