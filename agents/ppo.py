...
import os  # NEW: Import os to join paths correctly
...
class PPO(object):
    ...
    def __init__(self, args, env):
        ...
        self.actor = Actor(state_dim, args.emb_dim, action_dim, args.init_std)
        self.old_actor = Actor(state_dim, args.emb_dim, action_dim, args.init_std)
    ...
    def train(self):
        ...
        # NEW: Define the path for the CSV file
        csv_path = os.path.join(self.path, 'ppo_rewards.csv')
        
        # NEW: Open the CSV file to save rewards
        with open(csv_path, 'w', newline='') as csvfile:
            # NEW: Create a csv writer object
            csv_writer = csv.writer(csvfile)
            # NEW: Write the header row
            csv_writer.writerow(['Episode', 'Reward'])

            for i_episode in range(self.max_episode+1):
                disc_epi_reward, disc_factor, state = 0, 0.99, self.env.reset()
                while True:
                    action = self.choose_action(state)
                    state, reward, done, _ = self.env.step(action)
                    # The reward is the negative sum of AoIs, so we use it directly
                    # For average AoI, you would calculate reward / num_users
                ...
                # NEW: Write the episode number and the calculated reward to the CSV file
                csv_writer.writerow([i_episode, disc_epi_reward])
    ...
    def choose_action(self, state):
        ...
        # print(state) # This can be very verbose, consider removing for long runs
        mean = self.old_actor(state)
        
        """ This is for correlated actions """
    ...
    def update(self):
        ...
        for _ in range(self.upd_step):
            ...
            for i in range(n_data//self.mini_batch):
                if i == n_data//self.mini_batch - 1 and len(shuffle_idx) > self.mini_batch*i:
                    idx = shuffle_idx[self.mini_batch*i:]
                elif len(shuffle_idx) > self.mini_batch*(i+1):
