...
def channel_gain(distance_user_ris, distance_ris_bs, distance_user_bs, phase_shift, K_1=8, K_2=8):
    """ This function calculates the channel gains of user-k ---> RIS and RIS ---> BS """
    # DONE
    # ----------- Channel gain of user k -----> RIS 👇 ------------- #



    # Large-scale fading coefficients between user_k ---> RIS
    delta_user_ris = np.sqrt(L0 * ( distance_user_ris ** -2 ))  # 2 => is the free-space pathloss exponent 

  
    # Rayleigh fading(NLOS componets) 
    h_bar_user_ris = np.random.normal(loc = 0, scale = np.sqrt(2) / 2, size=(len(phase_shift), 2)).view(np.complex128) 

    # Deterministic fixed LoS component from user_k to RIS
    # h_tilda_user_ris  = h_tilda_user_ris 
    # print(len(phase_shift))
    # print(h_bar_user_ris.shape)

    # Small-scale fading coefficients
    h_hat_user_ris = np.sqrt(K_1 / (K_1 + 1)) * H_TILDA_USER_RIS  + np.sqrt(1 / (K_1 + 1)) * h_bar_user_ris
    h_user_ris = h_hat_user_ris * delta_user_ris  # Overall channel gain from user-k to RIS

    # ------------- Channel gain of RIS ---> BS 👇 --------------- #

    delta_ris_bs = np.sqrt(L0 * (distance_ris_bs ** -2 )) # 2 => is the free-space pathloss 
    ...
    # Deterministic fixed LoS component from RIS to BS
    # h_tilda_ris_bs = h_tilda_ris_bs

    # Small-scale fading coefficients
    h_hat_ris_bs = np.sqrt(K_2 / (K_2 + 1)) * H_TILDA_RIS_BS + np.sqrt(1 / (K_2 + 1)) * h_bar_ris_bs
    h_ris_bs = h_hat_ris_bs * delta_ris_bs  # Overall channel gain from RIS to BS

    # --------- Complete Channel Gain Calculation --------- #
    phase_shift_diag = np.diag(np.cos(phase_shift) + 1j * np.sin(phase_shift))  # Create diagonal matrix from phase shifts
    ...
    complete_channel_gain = h_user_bs + h_user_ris.conj().T @ phase_shift_diag @ h_ris_bs  # Applying phase shifts

    return complete_channel_gain
 

def SINR(k, power, distance_user_ris, distance_ris_bs, distance_user_bs, phase_shifts):
    # DONE
    ...
    h = channel_gain(distance_user_ris, distance_ris_bs, distance_user_bs,phase_shifts)
...
class TDMA(gym.Env):
    ...
    def __init__(self, n_users=2):
        ...
        self.n_message_parts = (self.n_users - 1) * 1 + 1  # 1 users with 2 parts, 1 user with 1 part
        ...
        self.transmit_power_low = np.zeros(1)
        self.transmit_power_high = np.ones(1) * 0.63  # or define appropriate max power in watts
        
        # Decoding orders: Continuous between (0, 1) for each message part
        self.transmition_order_low = np.zeros(self.n_message_parts)
        self.transmition_order_high = np.ones(self.n_message_parts)
        ...
        low = np.concatenate([self.transmit_power_low, self.transmition_order_low,self.phase_shift_low])
        high = np.concatenate([self.transmit_power_high, self.transmition_order_high, self.phase_shift_high])
        print(high)  
    ...
    def step(self, action):
        ...
        power = torch.clamp(action[0], 0, 0.63) # First 3 dimensions ReLU (0 to pmax = 100mw = 0.1w)
        ...
        # decoding_orders = torch.clamp(action[self.n_message_parts: 2* self.n_message_parts], 0, 1) # 3 different decoding orders
        transmition_order = torch.clamp(action[1: self.n_message_parts + 1], 0, 1)

        phase_shifts = torch.clamp(action[self.n_message_parts + 1:],0, 2*np.pi)

        # print(powers)

        max_idx_transmition = np.argmax(transmition_order)
        ...
        n_messages = self.n_message_parts

        # Calculate SINRS for each user messages
        # powers_copy = list(powers)
        # powers_real = list(powers)
  
        SINRS = [0] * n_messages
        SINRS[max_idx_transmition] = SINR(max_idx_transmition, power=power, distance_user_ris = self.USER_RIS_DISTANCES[max_idx_transmition], distance_ris_bs = 500, distance_user_bs = self.USER_BS_DISTANCES[max_idx_transmition], phase_shifts = phase_shifts)
        # for i in range(n_messages):
        #     max_idx_copy = np.argmax(powers_copy)
        #     max_idx = powers_real.index(powers_copy[max_idx_copy])


        #     SINRS[max_idx] = SINR(max_idx_copy, powers_copy = powers_copy, powers=powers, distance_user_ris = self.USER_RIS_DISTANCES[i], distance_ris_bs = 500, distance_user_bs = self.USER_BS_DISTANCES[i], phase_shifts = phase_shifts)
        #     powers_copy.pop(max_idx_copy)
