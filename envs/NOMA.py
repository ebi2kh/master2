...
def SINR(k, powers_copy, powers, distance_user_ris, distance_ris_bs, distance_user_bs, phase_shifts):
    # DONE
...
class NOMA(gym.Env):
    ...
    def __init__(self, n_users=2):
        ...
        low = np.concatenate([self.transmit_power_low, self.phase_shift_low])
        high = np.concatenate([self.transmit_power_high, self.phase_shift_high])
    ...
    def step(self, action):
        ...
        phase_shifts = torch.clamp(action[self.n_message_parts:],0, 2*np.pi)
        ...
        for i in range(n_messages):
            ...
            SINRS[max_idx] = SINR(max_idx_copy, powers_copy = powers_copy, powers=powers, distance_user_ris = self.USER_RIS_DISTANCES[max_idx], distance_ris_bs = 500, distance_user_bs = self.USER_BS_DISTANCES[max_idx], phase_shifts = phase_shifts)
